...
import '../widgets/payment_handler_widget.dart';
...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  String _getPaymentMethod() {
    if (selectedPaymentMethod == 'PAYMENT_COIN') {
      return 'PAYMENT_COIN';
    }
    ...
    if (selectedPaymentMethod == 'PAYMENT_COIN') {
      ...
      if (userCoinBalance < totalCoinsNeeded) {
        showTopSnackBar(
          ...
          CustomSnackBar.error(
            message:
            "Coinlaringiz yetarli emas. Kerakli: ${totalCoinsNeeded.toStringAsFixed(0)}, Mavjud: ${userCoinBalance.toStringAsFixed(0)}",
            backgroundColor: Colors.red,
            borderRadius: BorderRadius.circular(10.r),
          ),
        ...
        builder: (context, locationState) {
          return BlocBuilder<UserProfileBloc, UserProfileState>(
            builder: (context, userState) {
              final double userCoinBalance =
                  double.tryParse(
                    userState.user?.coinBalance?.toString() ?? '0',
                  ) ??
                      0.0;
              return Stack(
                children: [
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Text(
                                  selectedPaymentMethod == 'PAYMENT_COIN'
                                      ? "${CartService().getTotalCoins().toStringAsFixed(0)} coin"
